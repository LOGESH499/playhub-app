...
import type { VenueImageInput } from "@/lib/validators/venue.schema";
import {
  removeVenueMediaAction,
...
interface VenueGalleryUploadProps {
  ...
  onChange: (images: VenueImageInput[]) => void;
  disabled?: boolean;
}
...
export function VenueGalleryUpload({
  ...
  function handleUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    startTransition(async () => {
      setError(null);
      const result = await uploadVenueMediaAction(venueId, formData);
      if (result.error) {
        setError(result.error);
        return;
      }
      if (result.url && result.path) {
        const next: VenueImageInput[] = [
          ...images,
          {
            url: result.url,
            path: result.path,
  ...
  function removeImage(index: number) {
    ...
    startTransition(async () => {
      setError(null);
      const result = await removeVenueMediaAction(venueId, image.path);
      if (result.error) {
        setError(result.error);
        return;
      }
